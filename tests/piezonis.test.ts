/**
 * Piezoni's Pizza Accessibility Test Suite
 * 
 * This test suite performs automated accessibility testing for piezonis.com
 * using Playwright and axe-core. It generates Excel reports for any accessibility
 * violations found during testing.
 * 
 * Configuration is managed through the piezonis-fixture.ts file to ensure
 * consistent scanning rules across all tests.
 */

import { test, expect } from './fixtures/piezonis-fixture';
 ...
 * 
 * Note: Includes legacy location-specific paths (loc/warwick-pizza/, loc/catering-inquiry)
 * that require special handling for filename generation.
 */
const slugs = [
    "",
    "piezonis-cares",
...
const baseURL = 'https://piezonis.com';
...
for (let slug of slugs) {
    ...
    try {
        test(`visit ${url}`, async ({ page, axeScanner }, testInfo) => {
            ...
            // Add cache-busting query parameter to ensure we scan the latest version
            let url = baseURL + "/" + slug
            const separator = url.includes('?') ? '&' : '?';
            await page.goto(`${url}${separator}_cacheBust=${Date.now()}`, { waitUntil: 'networkidle' });

            /**
             * Run Accessibility Scan
             * 
             * Uses the Piezoni's-specific fixture with pre-configured:
             ...
             * Special slug handling for legacy location-specific URLs.
